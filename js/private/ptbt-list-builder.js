// wait until the end of the jQuery have loaded before running the code below
jQuery(document).ready(function($){
	...
	if( $('body[class*=" ptbt_"]').length || $('body[class*=" post-type-ptbt_"]').length ) {
		
		$ptbt_menu_li = $('#toplevel_page_ptbt_dashboard_admin_page');
	...
	*/
	...
	$('.file-id',$import_form_1).bind('change',function(){
		...
		var form_1_action_url = wpajax_url + '?action=ptbt_parse_import_csv';
		...
		$.ajax({
			...
			success: function( response ) {
				...
				if( response.status == 1 ) {
					...
					// get return html
					$return_html = ptbt_get_form_2_html( response.data );
					
					// update .ptbt-dynamic-content with the new return html
					$('.ptbt-dynamic-content',$import_form_2).html($return_html);
	...
	$(document).on('change','#import_subscribers #import_form_2 .ptbt-input',function(){
		
		setTimeout(function(){
			
			// if our form 2 validates
			if( ptbt_form_2_is_valid() ) {
	...
	$(document).on('click','#import_subscribers #import_form_2 .check-all',function(){
		...
		if( checked ) {
			...
			$('[name="ptbt_import_rows[]"]:not(:checked)',$import_form_2).trigger('click');
		
		} else {
		
			// trigger click on all inputs checked
			$('[name="ptbt_import_rows[]"]:checked',$import_form_2).trigger('click');
	...
	$(document).on('submit','#import_subscribers #import_form_2',function(){
		...
		var form_2_action_url = wpajax_url + '?action=ptbt_import_subscribers';
	...
	function ptbt_get_form_2_html( subscribers ) {
		...
		// prepare select html
		var select_fname = ptbt_get_selector('ptbt_fname_column',subscribers);
		var select_lname = ptbt_get_selector('ptbt_lname_column',subscribers);
		var select_email = ptbt_get_selector('ptbt_email_column',subscribers);
		...
		/// build row 1
		var row_1 = ptbt_get_form_table_row('Assign Data Column',assign_html);
		return_html += row_1;
		...
		$.each( subscribers, function(index,subscriber){
			...
			var th = '<th scope="row" class=" check-column"><input type="checkbox" id="cb-select-'+ row_id +'" name="ptbt_import_rows[]" class="ptbt-input" value="'+ row_id +'" /></th>';
			...
			$.each( subscriber, function(key,value){
				...
				var td = '<td>'+ value +'<input type="hidden" name="'+ field_name +'" class="ptbt-input" value="'+ value +'"></td>';
		...
		var row_2 = ptbt_get_form_table_row('Select Subscribers', table, 'Please select all the subscribers you\'d like to import.');
	...
	function ptbt_get_selector( input_id, subscribers ) {
		
		// setup our return variable
		var select = '<select name="'+ input_id +'" class="ptbt-input">';
	...
	function ptbt_get_form_table_row(label, input, description){
	...
	function ptbt_form_2_is_valid() {
		
		// setup our return variable
		var is_valid = true;
		
		// check if no subscribers are selected
		if( $('[name="ptbt_import_rows[]"]:checked',$import_form_2).length == 0 ) 
			is_valid = false;
			
		// check if no fname column is selected 
		if( $('[name="ptbt_fname_column"] option:selected',$import_form_2).val() == '' ) 
			is_valid = false;
			
		// check if no lname column is selected 
		if( $('[name="ptbt_lname_column"] option:selected',$import_form_2).val() == '' ) 
			is_valid = false;
			
		// check if no email column is selected 
		if( $('[name="ptbt_email_column"] option:selected',$import_form_2).val() == '' ) 
			is_valid = false;
